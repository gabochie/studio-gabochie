...
export async function onRequest(context) {
  ...
  var cors = { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'POST, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type, X-Admin-Key, X-CI-Secret' };
  ...
  try {
    ...
    var match;
    while ((match = locRegex.exec(sitemapText)) !== null) urls.push(match[1]);
    if (urls.length === 0) {
      await env.DB.prepare("UPDATE quality_runs SET status = 'failed', completed_at = datetime('now') WHERE id = ?").bind(runId).run();
