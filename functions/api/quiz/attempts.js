    ...
    if (!enrollment) {
      ...
      if (session) {
        enrollment = await env.DB.prepare(
          'SELECT e.id FROM enrollments e WHERE e.user_id = ? ORDER BY e.enrolled_at DESC LIMIT 1'
        ).bind(session.user_id).first();
        if (!enrollment) {
          enrollment = await env.DB.prepare(
            'SELECT e.id FROM enrollments e WHERE e.student_email = ? ORDER BY e.enrolled_at DESC LIMIT 1'
