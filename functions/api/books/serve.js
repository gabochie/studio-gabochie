export async function onRequest(context) {
  ...
  if (!authorized) {
    const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Access Required</title><style>body{font-family:system-ui,sans-serif;background:#0A1628;color:#CBD5E1;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;padding:20px}.card{background:#0F1E38;border:1px solid #1E3250;border-radius:16px;padding:40px 32px;max-width:420px;text-align:center}h1{font-size:28px;color:#F1F5F9;margin:0 0 8px}h1 span{color:#C9A84C}p{color:#6B7F9A;line-height:1.7;margin:0 0 24px}.btn{display:inline-block;background:#C9A84C;color:#0A1628;text-decoration:none;font-size:13px;font-weight:700;letter-spacing:.15em;text-transform:uppercase;padding:14px 32px;border-radius:8px}</style></head><body><div class="card"><h1>Access <span>Required</span></h1><p>' + (book.is_premium ? 'This book requires a valid purchase. Please visit the books page to purchase the premium bundle.' : 'Please submit your email on the books page to download this free book.') + '</p><a href="/books/" class="btn">Go to Books</a></div></body></html>';
  ...
  headers.set('Content-Disposition', `inline; filename="${book.file_url.split('/').pop()}"`);
